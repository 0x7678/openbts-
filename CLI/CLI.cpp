...
int sendsmsrpdu(int argc, char** argv, ostream& os, istream& is)
{
	if (argc!=3) return BAD_NUM_ARGS;
...
Parser::Parser()
{
	...
	addCommand("sendsmsrpdu", sendsmsrpdu, "<IMSI> <src> -- send pre-encoded SMS RPDU to <IMSI>, addressed from <src>, after prompting for RPDU hex string.");
	addCommand("sendsms", sendsms, "<IMSI> <src> <smsc> -- send SMS to <IMSI>, addressed from <src> with SMS-Center <smsc>, after prompting.");
